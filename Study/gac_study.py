...
# ============================================
# 공부용 복사본 — 원본: watertap/unit_models/gac.py
# 실제 실행용 아님
# ============================================
...
# Film Transfer Coefficient Kf ; fixed 혹은 calculated
...
# Surface Diffusion Coefficient Ds ; fixed 혹은 calculated
# enum은 사용할 수 있는 선택지를 정의하는 것
# auto() ; Enum은 원래 선택지마다 1, 2, 3... 이렇게 번호를 매겨야 하는데 auto()는 순서대로 알아서 매겨라~ 하는 것
# 즉 input = 1 surrogate =2 이렇게 하는 게 아니고 그냥 알아서 1, 2, 이렇게 해라
# 문자열 ("input", "surrogate") 이렇게 하지 않고 Enum 쓰는 이유는 유저 인풋 받을 때 typo 있으면 엉뚱하게 실행되지 않고 에러 띄우려고
# 즉 오타, 오설정 방지
...
class GACData(InitializationMixin, UnitModelBlockData):
    ...
    """
# GACData라는 새 클래스 만들기~ initialize & 기본 골격
# UnitModelBlockData ; IDAES에서 유닛 모델 기본 골격 제공해주는 부모 클래스

    CONFIG = ConfigBlock() # ConfigBlock으로부터 CONFIG 인스턴스 만듦
    #아래 코드들은 CONFIG 인스턴스에 옵션을 하나씩 등록해주는 것~
    ...
    **default** = False. The filtration unit does not support dynamic
    ...
    # dynamic = False. 즉 시간에 따라 상태가 변하는 모델이 아닌 steady-state으로 설정
    ...
    **default** - False. The filtration unit does not have defined volume, thus
    ...
    # holdup = False. holdup, 즉 bed 안에 고여있는 물질 양 고려하지 않는 것으로 설정 (시간에 따른 bed 안의 축적 계산 x)
    ...
    **PhysicalParameterObject** - a PhysicalParameterBlock object.}""",
    ...
    # physical parameter block이 맞는지 검증. property_package가 physical parameter block 타입인지 확인하라는 규칙
    # GAC는 여기에 MCASParameterBlock을 넣어야 함
    # MCASParameterBlock은 IDAES에서 PhysicalParameterBlock의 자식 클래스로, Multi-Component Aqueous Solution에 적합한 Block
    ...
    **Valid values:** {
    ...
    # implicit = True ; 원래 ConfigBlock은 기본적으로 implicit=false로, .declare()에서 미리 등록해둔 키만 받음
    # 근데 true로 하면 미리 등록 안 된 키가 와도 받아주는 것. 즉 새로운 키가 들어오면 domain=None짜리 ConfigValue 자동 생성해서 받아줌
    ...
    **MaterialBalanceType.total** - use total material balance.}""",
    ...
    # 물질수지(mass balance) 식 세우는 방식에 대한 옵션
    # default는 property package가 정한 기본값 따라가는 것 (=GAC에서는 MCAS)
    # domain은 MaterialBalanceType이라는 Enum (idaes.core에서 import)
    # none ; 물질수지 식을 만들지 않는다 (다른 데서 따로 처리하는 경우)
    # componentPhase ; (상, 성분) 조합마다 따로 balance 식 하나씩 (ex. (액체, 물) (기체, 물)..)
    # componentTotal ; 상은 합치고 성분마다 balance 식 하나씩
    # elementTotal ; 화합물이 아닌 원소(element) 단위로 balance (반응기 모델 등에서 쓴다)
    # total ; 성분 구분 없이 전체 질량 하나로 뭉뚱그린 balance
